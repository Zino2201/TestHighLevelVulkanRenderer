...
CB_ENABLE_FLAG_ENUMS(BackendFlagBits, BackendFlags);
...
{
public:
	explicit Backend(const BackendFlags& in_flags) : shader_language(ShaderLanguage::DXIL) { (void)(in_flags); }
	 ...
	 */
	virtual cb::Result<std::unique_ptr<BackendDevice>, std::string> create_device(ShaderModel in_requested_shader_model) = 0;
