...
/** Shader compiling eco+ */
std::vector<uint8_t> compile_shader(bool in_vs, std::vector<uint8_t> in_source)
{
...
void initialize_imgui()
{
...

	{
		...
		CB_ASSERTF(result.has_value(), "Failed to create ImGui UBO: {}", result.get_error());
	...
	/** Build font texture */
	{
		...
		CB_ASSERTF(tex_result.has_value(), "Failed to create ImGui font texture: {}", tex_result.get_error());
		...
		auto view_result = get_device()->create_texture_view(TextureViewInfo::make_2d(
			font_texture,
			Format::R8G8B8A8Unorm));
		CB_ASSERTF(view_result.has_value(), "Failed to create ImGui font texture view: {}", view_result.get_error());
		font_texture_view = view_result.get_value();
	}

	/** Create shaders */
	{
		auto vert_data = compile_shader(true, read_text_file("assets/shaders/imgui_vs.hlsl"));
		auto frag_data = compile_shader(false, read_text_file("assets/shaders/imgui_fs.hlsl"));

		// TODO: Change
		auto vert_result = get_device()->create_shader(ShaderInfo::make({ (uint32_t*) vert_data.data(), 
			(uint32_t*) vert_data.data() + vert_data.size() }));
		auto frag_result = get_device()->create_shader(ShaderInfo::make({ (uint32_t*) frag_data.data(), 
			(uint32_t*) frag_data.data() + frag_data.size() }));
	...
	/** Create pipeline layout */
	{
		...
		CB_ASSERTF(result.has_value(), "Failed to create ImGui pipeline layout: {}", result.get_error());
