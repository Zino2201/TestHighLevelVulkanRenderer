...
#define CB_ENABLE_FLAG_ENUMS(EnumType, FlagsType) \
	constexpr auto operator&(const cb::Flags<EnumType>& in_left, EnumType in_right) noexcept \
	{ \
		return in_left & cb::Flags<EnumType>(in_right); \
	} \
	constexpr auto operator&(EnumType in_left, EnumType in_right) noexcept \
	{ \
		return cb::Flags<EnumType>(in_left) & cb::Flags<EnumType>(in_right); \
	} \
	constexpr auto operator|(EnumType in_left, EnumType in_right) noexcept \
	{ \
		return cb::Flags<EnumType>(in_left) | cb::Flags<EnumType>(in_right); \
	...
	using FlagsType = cb::Flags<EnumType>;
...
{
	template<typename T> 
	struct hash<cb::Flags<T>>
	{
		uint64_t operator()(const cb::Flags<T>& in_flags) const
		{
			return std::hash<typename cb::Flags<T>::MaskType>()(static_cast<typename cb::Flags<T>::MaskType>(in_flags));
		}
	};
}